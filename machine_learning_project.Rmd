...
```
the in-sample error gives an accuracy of  

The second model is random forest
```{r cache=TRUE,collapse = TRUE,warning=FALSE}
system.time(modFit_rf<-train(classe~.,mehtod="rf",data=training,preProcess=c("center","scale","pca")))
modFit_rf$finalModel
```
the in-sample error gives an accuracy of  

The third model is boostrap with trees
```{r cache=TRUE,collapse = TRUE,warning=FALSE}
system.time(modFit_gmb<-train(classe~.,mehtod="gmb",data=training,preProcess=c("center","scale","pca")))
modFit_gmb$finalModel
```
the in-sample error gives an accuracy of  
